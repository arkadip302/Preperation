...
void solve(int x1, int y1, int x2, int y2, int moves, int n)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        if (safe(x1 + x[i], y1 + y[i], n) && (visi[x1][y1] + 1) < (visi[x1 + x[i]][y1 + y[i]]))
        {
            solve(x1 + x[i], y1 + y[i], x2, y2, visi[x1][y1] + 1, n);
        }
    }
}
int main()
